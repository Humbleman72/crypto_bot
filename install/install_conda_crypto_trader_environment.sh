...
# Automagic.
conda create --yes --name ${CONDA_ENV_NAME} --channel conda-forge pip python=${PYTHON_VERSION} && \
conda init bash && \
source /opt/conda/bin/activate ${CONDA_ENV_NAME} && \
if [[ "$(echo $PS1 | cut -d' ' -f1 | tr -d '()')" == "${CONDA_ENV_NAME}" ]]
then
    conda install --yes --channel conda-forge gcc gxx_linux-64 gcc_linux-64 cython numexpr bottleneck pandas=1.2.5 numpy scipy matplotlib tqdm setuptools-git paramiko ta-lib jupyterlab ipywidgets widgetsnbextension typeguard && \
