...
def convert_ohlcvs(to_asset, conversion_table, exchange_info, pair_paths=None):
    def convert_ohlcv_prices(from_asset, to_asset, conversion_table, exchange_info, pair_paths=None):
        if pair_paths is None:
            ...
            shortest_path = pair_paths['accuracy'][from_asset][to_asset]
        base_asset, quote_asset = shortest_path[0]
        to_asset = quote_asset if from_asset == base_asset else base_asset
    ...
    def convert_ohlcv_prices_helper(symbol, size, pair_paths=None):
        from_asset = get_base_asset_from_pair(symbol, exchange_info)
        if from_asset == to_asset:
            size = size[symbol].copy()
        else:
            size = convert_ohlcv_prices(
                from_asset, to_asset, size, exchange_info, pair_paths=pair_paths)
        return size
...
def convert_ohlcvs_from_pairs_to_assets(conversion_table, exchange_info, pair_paths=None):
    ...
    conversion_table = convert_ohlcvs(
        to_asset, conversion_table, exchange_info, pair_paths=pair_paths)
