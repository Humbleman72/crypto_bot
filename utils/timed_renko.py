...
class Renko:      
    ...
    def set_brick_size(self, HLC_history=None, auto=True, brick_size=10.0):
        self.brick_size = self.__get_optimal_brick_size(HLC_history.iloc[:, [0, 1, 2]]) if auto == True else brick_size
    ...
    def build_history(self, prices, is_timed=False):
        if len(prices) > 0:
            ...
            for p in self.source_prices[1:]:
                self.__renko_rule(p)
                if is_timed:
                    self.timed_renko_prices.append(self.renko_prices[-1])
    ...
    def evaluate(self, method='simple'):
        ...
        if method == 'simple':
            ...
            score = np.log(score + 1) * np.log(price_ratio) if score >= 0 and price_ratio >= 1 else -1.0
    ...
    def plot_renko(self, is_timed=False, col_up='g', col_down='r'):
        if is_timed:
            source_prices = self.source_prices
            timed_renko_prices = self.timed_renko_prices
            source_prices = source_prices.to_frame(name='close')
            timed_renko_prices = pd.Series(timed_renko_prices).to_frame(name='renko')
            timed_renko_prices.index = source_prices.index
            timed_renko_prices = pd.concat([source_prices, timed_renko_prices], axis='columns')
            timed_renko_prices.plot(figsize=(20, 10))
        else:
...
def get_renko_trigger(data, compress=False, direction_type='long', trigger_type='simple', 
    ...
    def evaluate_renko(brick, history, column_name):
        ...
        renko_obj.build_history(prices=history, is_timed=is_timed)
        return renko_obj.evaluate()[column_name]

    data.reset_index(drop=True)
    if compress:
        tr = data.ta.true_range(talib=True).dropna().reset_index(drop=True)
        optimal_bin_width = 0 if tr.size == 0 else 2 * iqr(tr) / tr.size ** (1.0 / 3)
        optimal_bin_count = 1 if optimal_bin_width == 0 else math.ceil((tr.max() - tr.min()) / optimal_bin_width)
    ...
    if method == 'brent':
        ...
        # Drop NaNs
        atr = talib.ATR(high=np.double(data.high), low=np.double(data.low), 
                        close=np.double(data.close), timeperiod=14)
    ...
    renko_obj_sfo.build_history(prices=data['close'], is_timed=is_timed)
    ...
    if return_raw:
        source_prices = renko_obj_sfo.source_prices
        timed_renko_prices = renko_obj_sfo.timed_renko_prices
        source_prices = source_prices.to_frame(name='close')
        timed_renko_prices = pd.Series(timed_renko_prices).to_frame(name='renko')
        timed_renko_prices.index = source_prices.index
        timed_renko_prices = pd.concat([source_prices, timed_renko_prices], axis='columns')
    ...
    if plot and len(renko_obj_sfo.get_renko_prices()) > 1:
        renko_obj_sfo.plot_renko(is_timed=is_timed)
    ...
    if not return_raw:
        if direction_type == 'long':
            ...
            if trigger_type == 'exit':
                ...
                if trigger_type == 'entry':
                    trigger = trigger and directions[-2] == 1
        elif direction_type == 'long_lagged':
            if trigger_type == 'exit':
                trigger = directions[-2] == -1 and directions[-3] == 1
            else:
                trigger = directions[-2] == 1
                if trigger_type == 'entry':
                    trigger = trigger and directions[-3] == -1
        elif direction_type == 'short_lagged':
            if trigger_type == 'exit':
                trigger = directions[-2] == 1 and directions[-3] == -1
            else:
                trigger = directions[-2] == -1
